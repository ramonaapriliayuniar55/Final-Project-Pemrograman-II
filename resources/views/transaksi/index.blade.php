    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                {{-- FORM SEARCH & FILTER --}}
                <form method="GET" action="{{ route('transaksi.index') }}" class="row g-2 mb-4">
                    <div class="col-md-5">
                                    ...
                                    <tr>
                                            ...
                                            @if($transaksi->status == 'Dipinjam')
                                                ...
                                                @if(\Carbon\Carbon::now()->startOfDay()->gt(\Carbon\Carbon::parse($transaksi->tanggal_kembali)->startOfDay()))
                                                    <span class="badge bg-danger d-block mt-1">
                                                        Terlambat {{ \Carbon\Carbon::parse($transaksi->tanggal_kembali)->startOfDay()->diffInDays(\Carbon\Carbon::now()->startOfDay(), true) }} Hari
                                                    </span>
                                                @endif
                                            @else
                                        ...
                                        <td>
                                            <div class="d-flex gap-1">
                                                ...
                                                <form action="{{ route('transaksi.destroy', $transaksi->id) }}" method="POST" class="form-hapus-transaksi m-0">
                                                    @csrf
                                                    @method('DELETE')
